...
def read(name, banlist,suffix):
	...
	to_write.write(prefix)
	nonce = 0
	count = 0 # make sure more than just clock updates
	while len(line) > 0: 
		if "#" in line[0] and count > 1:		
...
def make_spinfo(name, key):
	...
	cts.sort()
	open(name + "_ts.out","a").write("CONDTAINT REGS: " + str(cts) + "\n")
	return shadows
...
def do_all(name):
	...
	# for feasibility we knock of the family of lower case registers in the original design with a regex option to Daikon
	#system("java daikon.Daikon --var-omit-pattern=[b-ce-gi-np-rt-vx-z] " + name + "_2.decls " + name + "_2.dtrace " + shadows[0] + ".spinfo >" + name + "_" + shadows[0] + ".out")

	# remove temp files
	clean_up(name)

do_all("r_ref") # original assertion
do_all("r_min") # assert with conditions removed
do_all("r_clk") # min with flow changed to clk (a never)
do_all("r_con") # min with flow changed to a condtaint from depth_1
system("head -n -0 *_ts.out > results.txt")
system("rm *_ts.out")
