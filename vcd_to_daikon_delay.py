...
def make_decls(name,header):
	...
	for point in ["ppt ..tick():::ENTER\n  ppt-type enter\n","\nppt ..tick():::EXIT0\n  ppt-type subexit\n"]:
		...
		for reg in key: # exclude clock, add delay slot
			to_write.write("  variable " + reg[2].split("[")[0] + "_delay" + suffix)
		# add delay length
		to_write.write("  variable " + "delay" + suffix)
	# add delay slot to key
	out_key = key
	for k in key:
		out_key = out_key + [[k[0], "", k[2].replace("[","_delay[")]]	
...
def read(name):
	...
	while "#" not in line[0]: 
		...
		if i > -1:
			...
			key[i+regs] = key[i+regs] + [splits[0][1:]] # delay slot initialized to same initial value
		line = to_read.readline()
			...
			for index in range(regs):
				if (len(splits) > 1):
					if key[index][1] in splits[1]:
						i = index
			if i > -1:
				key[i+regs][3] = key[i][3] # capture delay
				key[i][3] = splits[0][1:]
				change = True
		line = to_read.readline()
